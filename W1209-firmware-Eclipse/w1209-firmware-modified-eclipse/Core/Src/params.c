...
void paramToString (unsigned char id, unsigned char* strBuff)
{
  switch (id) {
    case PARAM_RELAY_MODE:
      ...
      if ((paramCache[id] >= RELAY_MODE_C1) && (paramCache[id] <= RELAY_MODE_C3)) {
        ( (unsigned char*) strBuff) [0] = 'C';
        ( (unsigned char*) strBuff) [1] = '1' + paramCache[id];
        ( (unsigned char*) strBuff) [2] = 0;
      } else if ((paramCache[id] >= RELAY_MODE_H1) && (paramCache[id] <= RELAY_MODE_H3)) {
        ( (unsigned char*) strBuff) [0] = 'H';
        ( (unsigned char*) strBuff) [1] = '1' + paramCache[id] - RELAY_MODE_H1;
        ( (unsigned char*) strBuff) [2] = 0;
      } else if ((paramCache[id] >= RELAY_MODE_A1) && (paramCache[id] <= RELAY_MODE_A2)) {
        ( (unsigned char*) strBuff) [0] = 'A';
        ( (unsigned char*) strBuff) [1] = '1' + paramCache[id] - RELAY_MODE_A1;
        ( (unsigned char*) strBuff) [2] = 0;
      }
