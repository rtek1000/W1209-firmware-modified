...
void blinkLED(bool _blink) {
	setLED(state_pin || (_blink && state_waiting));
}
 ...
 */
void initRelay(void) {
	PA_ODR &= ~RELAY_BIT; // Relay Off
	PA_DDR |= RELAY_BIT;
	PA_CR1 |= RELAY_BIT;
}

void initLED(void) {
	PD_ODR |= LED_BIT; // LED Off
	PD_DDR |= LED_BIT;
	PD_CR1 |= LED_BIT;
}
...
void refreshRelay(void) {
	...
	unsigned int _RELAY_DELAY = getParamById(
			PARAM_RELAY_DELAY) * RELAY_TIMER_MULTIPLIER;

	bool _HYSTERESIS_UP = 0;
	bool _HYSTERESIS_DOWN = 0;
	bool _HYSTERESIS_DUAL = 0;
	...
	if (_RELAY_MODE == RELAY_MODE_A1) {
		...
		} else if ((_RELAY_MODE >= RELAY_MODE_H1)
				&& (_RELAY_MODE <= RELAY_MODE_H3)) {
			mode = true;
		...
		if (alarm_mode == true) {
			...
			if (state_toggle == true) { // Relay state is enabled
				...
				if (((_HYSTERESIS_DOWN == true) && (_temperature > _THRESHOLD))
						|| (((_HYSTERESIS_UP == true)
								|| (_HYSTERESIS_DUAL == true))
								&& (_temperature
										> (_THRESHOLD + _RELAY_HYSTERESIS)))) {
					incTimerEn = true; // timer++;
