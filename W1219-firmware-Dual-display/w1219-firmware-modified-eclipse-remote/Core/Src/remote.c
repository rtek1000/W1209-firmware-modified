...
void initSerialReceiver(void) {
	...
	PA_DDR &= ~BUTTON2_BIT;  // Pull-up with interrupt on pin PA.1
	PA_CR2 |= BUTTON2_BIT;   // Pull-up with interrupt on pin PA.1
	PA_CR1 |= BUTTON2_BIT;   // Pull-up with interrupt on pin PA.1

	EXTI_CR1 |= 0x02;   // PA generate interrupt on falling edge.
