...
void button_init(void) {
	PA_DDR &= ~(BUTTON1_BIT | BUTTON2_BIT); // input
	PA_CR1 |= (BUTTON1_BIT | BUTTON2_BIT);  // pull-up

	PD_DDR &= ~BUTTON3_BIT; // input
	PD_CR1 |= BUTTON3_BIT; // no pullup
...
void enableButton2interrupt(void) {
	PA_CR2 |= BUTTON2_BIT;   // Pull-up with interrupt on pin PC.3
}

void disableButton2interrupt(void) {
	PA_CR2 &= ~BUTTON2_BIT;   // Pull-up with Not interrupt on pin PC.3
}
