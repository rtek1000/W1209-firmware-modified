//Timer-T5 input capture (measure pulse width)
...
unsigned char guc_WaitFallEdge=0;     //Capture high and low flags
...
void TIMER5_Rpt(void) interrupt T5_VECTOR
{
	if(T5CON&0x40)                       //Determine whether it is an external event
	{
		...
		else
		{
			...
			TH5 = 0x00;
			TL5 = 0x00;	
			T5CON = 0x1B;						         //Clock divided by 8, 16-bit rising edge capture
			T5CON1 = 0x00;						         //T5 capture type selects the changing edge of T5 pin
			guc_WaitFallEdge = 0;
		}
	}
