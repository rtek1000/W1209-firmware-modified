...
void initSerialReceiver(void) {
	...
	PC_DDR &= ~BUTTON5_BIT;  // Pull-up with interrupt on pin PC.3
	PC_CR2 |= BUTTON5_BIT;   // Pull-up with interrupt on pin PC.3
	PC_CR1 |= BUTTON5_BIT;   // Pull-up with interrupt on pin PC.3

	EXTI_CR1 |= 0x20;   // PC generate interrupt on falling edge.
