...
void button_init(void) {
	PC_DDR &= ~(BUTTON1_BIT | BUTTON5_BIT); // input
	PC_CR1 |= BUTTON1_BIT | BUTTON5_BIT;  // pull-up

	PB_DDR &= ~(BUTTON2_BIT | BUTTON4_BIT); // input
	PB_CR1 |= BUTTON2_BIT | BUTTON4_BIT;    // pull-up

	PA_DDR &= ~BUTTON3_BIT; // input
	PA_CR1 |= BUTTON3_BIT;  // pull-up

	PD_DDR &= ~BUTTON6_BIT; // input
	PD_CR1 |= BUTTON6_BIT;  // pull-up
...
void setButton6stateHIGH(void) {
	/* Pull-up with interrupt */
	PD_DDR &= ~BUTTON6_BIT;
}

void setButton6stateLOW(void) {
    /* LOW */
	PD_ODR &= ~BUTTON6_BIT;
	/* Push-pull, fast mode */
	PD_DDR |= BUTTON6_BIT;
...
void enableButton5interrupt(void) {
	PC_CR2 |= BUTTON5_BIT;   // Pull-up with interrupt on pin PC.3
}

void disableButton5interrupt(void) {
	PC_CR2 &= ~BUTTON5_BIT;   // Pull-up with Not interrupt on pin PC.3
}

bool get_Button1(void) {
	// return ((PC_IDR & BUTTON1_BIT) == BUTTON1_BIT);
	return ((BUTTONS_PORT1 >> BUTTON1_POS) & 1);
}
...
//		;
	...
	bool state = ((BUTTONS_PORT2 >> BUTTON2_POS) & 1);
//
...
//		;
	...
	return ((BUTTONS_PORT3 >> BUTTON3_POS) & 1);
}

bool get_Button4(void) {
	return ((BUTTONS_PORT4 >> BUTTON4_POS) & 1);
}

bool get_Button5(void) {
	return ((BUTTONS_PORT5 >> BUTTON5_POS) & 1);
}

//bool get_Button6(void) {
//	return ((BUTTONS_PORT6 >> BUTTON6_POS) & 1);
//}
